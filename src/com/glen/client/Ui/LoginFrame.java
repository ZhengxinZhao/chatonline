...
public class LoginFrame extends JFrame implements ActionListener {
	...
	public LoginFrame(TcpClient client) {
		this.client = client;
		...
		//处理北部背景图片标签
		jlb_north = new JLabel(new ImageIcon("res/image/login/login.jpg"));
		jlb_north.setBounds(0,0,430,182);
		c.add(jlb_north);
		//处理右上角最小化和关闭按钮
		btn_min = new JButton(new ImageIcon("res/image/login/min.jpg"));
		...
		btn_exit = new JButton(new ImageIcon("res/image/login/exit.png"));
		...
		//处理中部账号密码框左边企鹅图片标签
		jlb_photo = new JLabel(new ImageIcon("res/image/login/touxiang.png"));
		...
		c.add(autoLog);
		//处理南部登录按钮
		btn_login = new JButton(new ImageIcon("res/image/login/loginbutton.png"));
		...
		this.setIconImage(new ImageIcon("res/image/login/Q.png").getImage());//修改窗体默认图标
	...
	public void onLoginFailed(String errorInfo){
		JOptionPane.showMessageDialog(null, errorInfo, "登陆失败", JOptionPane.WARNING_MESSAGE);
	}
}
